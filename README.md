...
**Checked against the companion clo-atlas project (2026-07-12), see `research/data_reuse_assessment.md`:** clo-atlas's `edgar_resolved_issuers.parquet` + `bdc_soi_positions.parquet` already supply the entity-resolution and per-position coupon/spread/fair-value half of this input, real not synthetic — but it's a BDC loan book (a same-universe proxy), not a CLO's own collateral tape, and it carries no agency-rating or industry-classification column, so it cannot feed the WARF/diversity-score engines in Phases 1-2 either way. clo-atlas's FINRA TRACE CLO secondary-pricing scrape is real, current, and outside any scraping guardrail; it's wired into the research notebook (Section 7) as a spread-assumption anchor, and would become a genuine methodology cross-check once it accretes a time series spanning the Apr/Jun 2026 announcement dates.
